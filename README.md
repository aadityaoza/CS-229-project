...
7. Run `python3 kernel_svm.py <path/to>/transfer.csv` - This file runs an RBF-kernel SVM (`SVC`) with a fast class-weight sweep (10 iterations, `cw[1]=2**it`) and writes precision/recall/f1 per iteration to `./prf/kernel_svm_transfer_prf.txt` and saves models to `./models/kernel_svm_transfer_<classweight>.sav`. For the deep sweep, run `python3 kernel_svm.py <path/to>/transfer.csv --sweep linear --max-iters 513`. To train a single model for a specific weight (replacing `kernel_svm_model.py`), run `python3 kernel_svm.py <path/to>/transfer.csv --weight 32`. Same experiment can be repeated with `cash_out.csv` as well.
